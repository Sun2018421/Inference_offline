...
def disparity_regression(x, maxdisp, disp_values):
    assert len(x.shape) == 4
    #disp_values = torch.linspace(0, maxdisp-1, maxdisp).half().to(ct.mlu_device())
    print(sys._getframe().f_code.co_name, disp_values.cpu().shape)
    disp_values = disp_values.view(1, maxdisp, 1, 1)
    return torch.sum(x * disp_values, 1, keepdim=False)

def disparity_variance(x, maxdisp, disparity, disp_values):
...
def build_concat_volume(refimg_fea, targetimg_fea, maxdisp):
    B, C, H, W = refimg_fea.shape  # 1, 12, 64, 128
    volume = torch.zeros([B, 2 * C, maxdisp, H, W]).half().to(ct.mlu_device())  # [1, 24, 16, 64, 128]
    #volume = torch.zeros([B, 2 * C, maxdisp, H, W]).to(ct.mlu_device())
    res = []
    for i in range(maxdisp):
        if i > 0:
            res.append(torch.cat([
                torch.cat([volume[:, :C, i, :, 0:i].unsqueeze(2), refimg_fea[:, :, :, i:].unsqueeze(2)],dim=4),
                torch.cat([volume[:, C:, i, :, :i].unsqueeze(2), targetimg_fea[:, :, :, :-i].unsqueeze(2)],dim=4)
                ], dim = 1))
        else:
            res.append(torch.cat([refimg_fea.unsqueeze(2), targetimg_fea.unsqueeze(2)], dim = 1))
    volume = torch.cat(res, dim = 2)
    return volume
...
def groupwise_correlation(fea1, fea2, num_groups):
    ...
    cost = (fea1 * fea2).view([B, num_groups, channels_per_group, H, W])  # [1, 40, 4, 64, 128]
    cost = cost.sum(2) / channels_per_group
...
def groupwise_correlation_4D(fea1, fea2, num_groups):
    ...
    assert C % num_groups == 0
    channels_per_group = C // num_groups
    cost = (fea1 * fea2).view([B, num_groups, channels_per_group, D, H, W])
    cost = cost.sum(2) / channels_per_group
...
def build_gwc_volume(refimg_fea, targetimg_fea, maxdisp, num_groups):
    ...
    volume = torch.zeros([B, num_groups, maxdisp, H, W]).half().to(ct.mlu_device())
    res = []
    for i in range(maxdisp):
        if i > 0:
            res.append(torch.cat([volume[:, :, i, :, 0:i].unsqueeze(2), groupwise_correlation(refimg_fea[:, :, :, i:], targetimg_fea[:, :, :, :-i], num_groups).unsqueeze(2)], dim=4))
        else:
            res.append(groupwise_correlation(refimg_fea, targetimg_fea, num_groups).unsqueeze(2))
    res_all = torch.cat(res, dim = 2)
    return res_all
...
class UniformSampler(nn.Module):
    ...
    def forward(self, min_disparity, max_disparity, number_of_samples=10, range_multiplier = None):
        """
        Args:
        ...
        # range_multiplier = torch.linspace(1.0, number_of_samples, number_of_samples).view(number_of_samples, 1, 1).half().to(ct.mlu_device())
        range_multiplier = range_multiplier.view(number_of_samples, 1, 1)
        print(sys._getframe().f_code.co_name, range_multiplier.cpu().shape)
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, left_input, right_input, disparity_samples):
        ...
        left_feature_map = left_input.unsqueeze(0).repeat(disparity_samples.size()[1], 1, 1, 1, 1).permute([1, 2, 0, 3, 4])
        right_feature_map = right_input.unsqueeze(0).repeat(disparity_samples.size()[1], 1, 1, 1, 1).permute([1, 2, 0, 3, 4])

        left_y_coordinate = torch.linspace(0.0, left_input.size()[3]-1, left_input.size()[3]).half().to(ct.mlu_device())  # [256]  -> 0.1.2...255
        left_y_coordinate = left_y_coordinate.repeat(left_input.size()[2]).view(left_input.size()[2], left_input.size()[3]) # [128, 256]   -> 0.1.2...255
        ...
        right_y_coordinate = left_y_coordinate.unsqueeze(0).repeat(disparity_samples.size()[1], 1, 1, 1).permute([1, 0, 2, 3]) - disparity_samples
        ...
        gather_index = right_y_coordinate.unsqueeze(0).repeat(right_input.size()[1], 1, 1, 1, 1).permute([1, 0, 2, 3, 4]).cpu().long() # [1, 12, 16, 128, 256]
        # print(gather_index.shape, gather_index)
        # torch.Size([1, 12, 16, 128, 256]) 
        # torch.Size([1, 160, 16, 128, 256])
        # torch.Size([1, 6, 12, 256, 512])
        # torch.Size([1, 80, 12, 256, 512])
        warped_right_feature_map = torch.gather(right_feature_map, dim=4, index=gather_index)


        warped_right_feature_map = ( - ((right_y_coordinate_1 < 0) +(right_y_coordinate_1 > right_input.size()[3] - 1)) + 1.0) *  (warped_right_feature_map)
